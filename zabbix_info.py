...
#生成文件名称：
xlsfilename = 'host_info_%s.xlsx' % time.strftime("%Y-%m-%d", time.localtime())
...
#    ['swap平均剩余(单位G)','trends_uint','system.swap.size[,free]','avg','',1048576000],
    ...
    ['data区总大小(单位G)','trends_uint','vfs.fs.size[/data,total]','avg','',1073741824],
    ['data分区平均剩余(单位G)','trends_uint','vfs.fs.size[/data,free]','avg','',1073741824],
...
class ReportForm:
    ...
    def __init__(self):
        '''打开数据库连接'''
        self.conn = MySQLdb.connect(host=zdbhost,user=zdbuser,passwd=zdbpass,port=zdbport,db=zdbname,charset="utf8")
        self.cursor = self.conn.cursor(cursorclass=MySQLdb.cursors.DictCursor)
 
        #生成zabbix哪个分组报表
        for group in ['001-市直属分中心']:
    ...
    def __getHostList(self):
        ...
        for i in hostlist:
            hostid = i['hostid']
            sql = '''select name from hosts where status = 0 and hostid = '%s' ''' % hostid
            ret = self.cursor.execute(sql)
            if ret:
                IpInfoList[self.cursor.fetchone()['name']] = {'hostid':hostid}
        return IpInfoList
    ...
    def getLastMonthData(self,type,hostid,table,itemname):
        ...
        lst_last = datetime.date(datetime.date.today().year,datetime.date.today().month,4)-datetime.timedelta(1)
    ...
    def writeToXls2(self):
        ...
        worksheet.write(0,0,"主机",format1)
        i = 1
